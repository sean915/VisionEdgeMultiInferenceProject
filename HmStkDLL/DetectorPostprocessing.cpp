...
namespace HMSTACK {
    ...
    void MapTriggerOutsToJob_Letterbox_Trt(
        ...
        float nms_iou_thr,
        HMSTACK::Detector::DefectJob& job)
    {
        ...
        job.trig.cell = {};
        job.trig.pnp = {};
        job.trig.cell_score = 0.f;
        job.trig.pnp_score = 0.f;
        ...
        const bool found_cell = (best0_score >= 0.f);
        const bool found_pnp = (best1_score >= 0.f);

        if (!found_cell && !found_pnp) return;
        ...
        if (found_pnp) {
            ...
            cv::Rect r = clamp_rect2f_to_int_rect(rf, frameW, frameH);
            if (r.area() > 0) {
                job.trig.pnp_score = k.score;
                job.trig.pnp = r;
            }
        }

        const bool has_cell = job.trig.cell.area() > 0;
        const bool has_pnp = job.trig.pnp.area() > 0;

        job.trig.ok =
            has_cell && has_pnp &&
            (job.trig.cell_score >= cell_min_score) &&
            (job.trig.pnp_score >= pnp_min_score);
    }
        ...
        HMSTACK::Detector::DefectJob& job)
    {
        ...
        for (const auto& k : kept) {
            ...
            if (k.cls == 0) {
                if (k.score >= job.trig.cell_score) {
                    job.trig.cell_score = k.score;
                    job.trig.cell = r;
                }
            }
            else if (k.cls == 1) {
                if (k.score >= job.trig.pnp_score) {
                    job.trig.pnp_score = k.score;
                    job.trig.pnp = r;
                }
            }
    ...
    // - DLL 콜백으로 전달할 때 사용
    ResultItemC ToCResult(const HMSTACK::ResultItem& r)
    {
        ResultItemC c{};
