...
namespace HMSTACK {
  ...
  // - 반환값: 성공(true) / 실패(false)
    bool ParseDefectProbs3_Ort(const std::vector<Ort::Value>& outs, float& p_ab, float& p_no, float& p_em)
    {
        p_ab = p_no = p_em = 0.f;
        ...
        // 출력 순서: [abnormal, normal, empty]
        p_ab = data[0]; p_no = data[1]; p_em = data[2];
        return true;
    }
  ...
  // - 반환값: 성공(true) / 실패(false)
    ...
    bool ParseDefectProbs3_Trt(
        ...
        float& p_ab, float& p_no, float& p_em)
    {
        p_ab = p_no = p_em = 0.f;
        ...
        if (isFp32) {
            if (outFp32.size() < 3) return false;
            p_ab = outFp32[0]; p_no = outFp32[1]; p_em = outFp32[2];
            return true;
        }
        else {
            if (outFp16.size() < 3) return false;
            p_ab = fp16_to_fp32(outFp16[0]);
            p_no = fp16_to_fp32(outFp16[1]);
            p_em = fp16_to_fp32(outFp16[2]);
            return true;
        }
    }
