...
public:
    ...
    int run();   // 워커 스레드 시작 (triggerLoop, defectLoop)
    int stop();  // 워커 스레드 중지 (재시작 시 Initialize 없이 run만 호출)
...
private:
    ...
    std::filesystem::path triggerModelPath;
    std::filesystem::path defectModelPath;
