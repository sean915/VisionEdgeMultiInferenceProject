...
namespace HmCutter
{
    Detector::Detector(const AlgorithmConfig& config)
        : config_(config)
    {
        triggerModelPath = config.triggerModelPath;
        defectModelPath = config.defectModelPath;
    ...
    // ------------------------------------------------------------
    // 모델 메타(JSON) 로드 + 세션 생성 (워커 스레드 시작은 run()에서)
    void Detector::model_setup() {
        status_ = AlgorithmStatus::INITIALIZING;

        try {
            const std::filesystem::path modelDir =
                L"D:/workspace/IVS.SW.INFERENCE.cppNcsharp/InferenceClient/inferenceClient/x64/Debug/cuttermagazine_model";
    ...
    // run / stop
    // ------------------------------------------------------------
    // Initialize는 모델 로드만 수행. run()으로 워커 시작, stop()으로 중지.
    // 멈춤 후 재시작 시 재초기화 없이 run()만 호출.
    ...
    void Detector::triggerLoop()
    {
        ...
        // Python: conf_threshold
        const float conf_thr = 0.4;

        // Python 코드에서 iou_threshold를 따로 안 넘기면(=None) Ultralytics 기본값을 쓰지만,
        // 현재 C++ 파서(MapTriggerOuts...)는 nms_iou_thr 인자를 요구하므로 기존 값을 유지.
    ...
    void Detector::defectLoop()
    {
        ...
        while (running_)
        {
            ...
            // ===== 이하 분류/시각화/콜백 로직은 원본 그대로 =====
            const float MIN_VALID = 0.3f;
            const float SURE = 0.7f;
            ...
            const char* final_str = "Error";
            std::string reason = "parse_failed";

            if (ok_probs) {
                if (topScore < MIN_VALID) {
                    final_str = "Error";
                    reason = "top1_below_min_valid";
                }
                else if (topScore < SURE) {
                    final_str = "Questionable";
                    reason = "top1_between_min_valid_and_sure";
                }
                else {
                    if (top == AB) {
                        final_str = "Abnormal";
                        reason = "sure_abnormal";
                    }
                    else {
                        final_str = "Normal";
                        reason = "sure_normal_or_empty";
                    }
                }
            }
